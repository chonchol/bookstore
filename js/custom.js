...
function displayBooks(books) {
  ...
  if (books.length === 0) {
    container.innerHTML =
      '<p class="text-center p-5 mx-auto w-full text-2xl">No books found with the given Title!</p>';
    return;
  }
  ...
  books.forEach((book) => {
    ...
    const savedBooks =
      JSON.parse(localStorage.getItem("wishlistedBooks")) || [];
    const isWishlisted = savedBooks.some(
      (savedBook) => savedBook.id === bookID
    );
    ...
    bookDiv.innerHTML = `
        <div>
            ...
            <div class="pt-5 flex items-start justify-between w-full">
                <a href="book-details.html" class="font-bold hover:text-black text-[18px] w-4/5 book-title" data-book-id="${bookID}">${
      book.title
    }</a>
                <div class="flex justify-end w-1/5 mt-1">
                    <img class="w-5 h-5 cursor-pointer love-icon" data-book-id="${bookID}" src="${
      isWishlisted ? `images/heart-solid.svg` : `images/heart-regular.svg`
    } " alt="">
                </div>
            </div>
